...
module.exports = function(licenses, opt){
  ...
  if (opt.show.includes('packages')) {
    table(data, {
      'Module name': 25 * ( opt.width || 1),
      'package': 14 * (opt.width || 1),
      'License': 14 * (opt.width || 1),
      'README': 14 * (opt.width || 1)
  ...
  if (opt.show.includes('licenses')) {
    table(count, {
      License: 40 * (opt.width || 1),
      Number: 12 * (opt.width || 1),
      '%': 12 * (opt.width || 1)
  ...
  if (opt.show.includes('reports')) {
    table(facts, [70 * (opt.width || 1)], Object.assign({ title: 'Reports', margin: 3 }, opt));
  }
}
